...
| Softmax / argmax over an axis | `softmax(logits, axis=-1)` or `np.argmax(logits, axis=1)` | `let p[b, class] = softmax[class](logits[b, class]);`<br>`let pred = argmax[class](p);` | [Coordinate-aware functions](https://github.com/einlang/einlang/blob/main/docs/reference.md#coordinate-aware-functions) |
| Normalize over an axis | `F.layer_norm(x, (128,))` or `(x - x.mean(1)) / x.std(1)` | `let y = normalize_over[feature](x[batch, time, feature]);`<br>`normalize_over[time]` and `normalize_over[batch]` differ only in the bracket name | [Coordinate-aware functions](https://github.com/einlang/einlang/blob/main/docs/reference.md#coordinate-aware-functions) |
| Chained axis ops | `best = np.argmax(np.sum(W[:,:,None] * X[None,:,:], axis=-1), axis=-1)`<br>`total = np.sum(np.max(np.min(T, axis=2), axis=1), axis=0)` | `let best[i] = argmax[hidden](sum[k](W[i, k] * X[k, hidden]));`<br>`let total = sum[i](max[j](min[k](T[i, j, k])));` — coordinates compose, no axis re-counting | [Coordinate-aware functions](https://github.com/einlang/einlang/blob/main/docs/reference.md#coordinate-aware-functions) |
