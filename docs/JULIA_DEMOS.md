...
- **Numerics (reusable modules):** [numerics/](../examples/numerics) — DiffEq (Euler decay stepper + trajectory), Optim (gradient descent 2D), QuantEcon (Bellman value iteration). Use from your code via `use numerics::diffeq`; run demo: `python3 -m einlang examples/run_numerics.ein`. See [numerics/README](../examples/numericsREADME).
- **Parameter estimation & workflows:** [applications/](../examples/applications) — **decay_calibration.ein** (fit then forecast); **markov_credit.ein** (steady-state of a credit-rating transition model). [value_iteration/](../examples/value_iteration) — **policy_iteration.ein** (Howard: policy evaluation + improvement). Same patterns as SciML, Optim.jl, QuantEcon, and [JuliaHub case studies](https://juliahub.com/case-studies).
