...
- **Numerics (reusable modules):** [numerics/](../../examples/numerics/) — DiffEq (Euler decay stepper + trajectory), Optim (gradient descent 2D), QuantEcon (Bellman value iteration). Use from your code via `use numerics::diffeq`; run demo: `python3 -m einlang examples/run_numerics.ein`. See [numerics/README](../../examples/numerics/README).
