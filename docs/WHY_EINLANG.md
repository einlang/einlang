...
**Autodiff note:** We do not bolt on AD with helper APIs; Einlang compiles derivative syntax directly into executable IR for tensor programs, so supported sensitivities are computed by the compiler instead of by finite-difference probes.
...
- **Gradients without gradient code** — `@loss / @w` is the gradient. The compiler derives it; you never write backprop or VJPs by hand. Same mechanism for training, sensitivity analysis, and adjoints, and a cleaner replacement for finite-difference gradients where the operation is supported.
...
- **Not “gradient as a separate API”** — You don’t call `gradient(f, x)` or `jax.grad(f)(x)`. You write `@loss / @w` where the math is. One mechanism for all derivatives, and usually the first thing to reach for instead of finite differences when you need sensitivities.
