...
def test_nonzero_all_ranks(compiler, runtime):
    ...
    # Format: 2D array [rank, num_nonzero] where each column is an index tuple
    # NumPy nonzero returns (row_indices, col_indices), convert to expected format
    expected_2d = np.column_stack(expected_2d_indices).T  # Shape: [2, num_nonzero]
    ...
    # Normalize to expected format [rank, num_nonzero]
    if actual_2d_arr.ndim == 2 and actual_2d_arr.shape[1] == 2:
        actual_2d = actual_2d_arr.T
    elif actual_2d_arr.ndim == 2 and actual_2d_arr.shape[0] == 2:
        actual_2d = actual_2d_arr
    elif actual_2d_arr.ndim == 1:
        num_nonzero = np.count_nonzero(x_2d)
        actual_2d = actual_2d_arr.reshape(2, -1)
    else:
        raise AssertionError(f"Unexpected shape for nonzero_2d: {actual_2d_arr.shape}")
    sort_idx_expected = np.lexsort(expected_2d[::-1])
    sort_idx_actual = np.lexsort(actual_2d[::-1])
    expected_sorted = expected_2d[:, sort_idx_expected]
    actual_sorted = actual_2d[:, sort_idx_actual]
    ...
    # Format: 2D array [rank, num_nonzero] where each column is an index tuple
    expected_3d = np.column_stack(expected_3d_indices).T  # Shape: [3, num_nonzero]

    actual_3d = result.outputs['nonzero_3d']
    actual_3d_arr = np.array(actual_3d)
    # Normalize to expected format [rank, num_nonzero]
    if actual_3d_arr.ndim == 2 and actual_3d_arr.shape[1] == 3:
        actual_3d = actual_3d_arr.T
    elif actual_3d_arr.ndim == 2 and actual_3d_arr.shape[0] == 3:
        actual_3d = actual_3d_arr
    elif actual_3d_arr.ndim == 1:
        num_nonzero_3d = np.count_nonzero(x_3d)
        actual_3d = actual_3d_arr.reshape(3, -1)
    else:
        raise AssertionError(f"Unexpected shape for nonzero_3d: {actual_3d_arr.shape}")
    sort_idx_expected = np.lexsort(expected_3d[::-1])
    sort_idx_actual = np.lexsort(actual_3d[::-1])
    expected_sorted = expected_3d[:, sort_idx_expected]
    actual_sorted = actual_3d[:, sort_idx_actual]
    np.testing.assert_array_equal(actual_sorted, expected_sorted)
