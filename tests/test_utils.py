...
def ensure_test_dependency(import_name: str, *, package_spec: Optional[str] = None, timeout: int = 600):
    """Import a test dependency or raise a setup hint for the test environment."""
    ...
    try:
        ...
        raise RuntimeError(
            f"missing test dependency '{import_name}'. "
            f"Install the test extras with `pip install -e \".[test]\"` "
            f"or install `{package_spec}` manually."
        ) from exc
