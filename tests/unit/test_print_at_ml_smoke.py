...
ML_ACTIVATION_PRINT_AT_GOLDEN_CASES: List[Tuple[str, str, str]] = [
    (
        'relu',
        '\n\nuse std::ml;\nlet x = 1.0;\nlet y = std::ml::relu(x);\nprint(@y);\n',
        'let @y = {\n    let _@relu_x: f32 = if x > 0.0 { @x } else { 0.0 };\n    _@relu_x\n};',
    ),
    (
        'sigmoid',
    ...
    (
        ...
        'let @y = {\n    let _@relu6_x: f32 = if x <= 0.0 { 0.0 } else if x >= 6.0 { 0.0 } else { @x };\n    _@relu6_x\n};',
    ),
    (
        'prelu',
    ...
    (
        ...
        'let @y = {\n    let shifted = x + 3.0;\n    let _@shifted = @x;\n    let _@clamped = if shifted < 0.0 { 0.0 } else if shifted > 6.0 { 0.0 } else { _@shifted };\n    _@clamped / 6.0\n};',
    ),
    (
        'softplus',
