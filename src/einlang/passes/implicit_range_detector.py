...
class _SubstituteDefidsInExprVisitor(IRVisitor[ExpressionIR]):
    ...
    def _loc(self, node: Any) -> SourceLocation:
        return node.location or SourceLocation("<generated>", 0, 0, 0, 0)
...
class ImplicitRangeDetector(IRVisitor[None]):
            ...
            try:
                ...
                if array_defid is not None and decl is not None and array_defid == decl.defid:
                    ...
                    if shape is None:
                        ...
                        loc = decl.location or SourceLocation('<unknown>', 0, 0, 0, 0)
                ...
                if shape is None and base_expr is not None and isinstance(base_expr, IdentifierIR):
                    ...
                    shape = RectangularAccessIR(
                        ...
                        indices=[LiteralIR(value=index_position, location=base_expr.location, type_info=_PT(name='i32'))],
                        location=base_expr.location,
                    )
                if shape is not None:
    ...
    def _get_array_shape(self, array_defid: DefId, index_position: int) -> Union[int, ExpressionIR, None]:
        ...
        type_info = getattr(var_def, 'param_type', None) or var_def.type_info  # FunctionValueIR vs other
    ...
    def _extract_constraint_from_array_access_ir(
        ...
        """Extract constraint and solve for target variable by DefId."""
        target_defid = getattr(self, '_target_defid', None)
        if target_defid is None:
            return None
        shape_desc = shape if isinstance(shape, int) else "<dynamic>"
        ...
        range_result = solve_index_constraint(index_expr, target_var, dimension_bound, target_defid=target_defid)
    ...
    def _to_output_range_ir(self, range_result: RangeInfo) -> Optional[RangeInfo]:
        ...
        start_defids = _collect_defids(range_result.start)
        end_defids = _collect_defids(range_result.end)
        all_defids = start_defids | end_defids
        if not all_defids:
            return range_result
        ...
        new_start = self._substitute_defids_in_expr(range_result.start, substitute_map)
        new_end = self._substitute_defids_in_expr(range_result.end, substitute_map)
        if new_start is range_result.start and new_end is range_result.end:
            return range_result

        return DynamicRange(start=new_start, end=new_end)
    ...
    def infer_clause_index_range_from_reduction_body(
        ...
        if not isinstance(red, ReductionExpressionIR) or red.body is None:
            return None
        body = red.body
        loc = location or clause.location or red.location
        if loc is None:
            loc = SourceLocation('', 0, 0)
        ...
        try:
            ...
            for access in accesses:
                ...
                for idx_pos, idx in enumerate(indices_flat):
                    ...
                    dimension_bound = shape
                    range_result = solve_index_constraint(idx, target_var, dimension_bound, target_defid=target_defid)
    ...
    def get_range_from_where_clause(self, where_clause: Any, defid: Optional[DefId] = None) -> Any:
        if not where_clause:
            return None
        if where_clause.ranges and where_clause.ranges and defid is not None:
            ...
            if range_obj is not None:
                ...
                if isinstance(range_obj, range):
                    return range_obj
        if not where_clause.constraints or not where_clause.constraints or defid is None:
            return None
    ...
    def _solve_index_constraint_ir(
        ...
        def _is_target(expr: ExpressionIR) -> bool:
            return isinstance(expr, (IdentifierIR, IndexVarIR)) and expr.defid == target_defid

        def contains_target(expr: ExpressionIR) -> bool:
            if isinstance(expr, (IdentifierIR, IndexVarIR)):
                return _is_target(expr)
        ...
        def solve(expr: ExpressionIR, bound: ExpressionIR) -> Optional[ExpressionIR]:
            if isinstance(expr, (IdentifierIR, IndexVarIR)):
                return bound if _is_target(expr) else None
            ...
            if op == BinaryOp.SUB:
                ...
                return None
            if op == BinaryOp.MUL:
                if _is_target(left) and not contains_target(right):
                    one = LiteralIR(value=1, location=loc, type_info=infer_literal_type(1))
                ...
                if _is_target(right) and not contains_target(left):
                    one = LiteralIR(value=1, location=loc, type_info=infer_literal_type(1))
    ...
    def _build_output_range_end_from_constraint(
        ...
        return self._solve_index_constraint_ir(constraint_idx, target_defid, shape_dim, loc)
    ...
    def infer_range_from_array_access(
        ...
        for expr_node in accesses:
            ...
            def _is_target(e: ExpressionIR) -> bool:
                return isinstance(e, (IdentifierIR, IndexVarIR)) and e.defid == target_defid
