...
The lowering pass handles the backward pass automatically. When the analysis annotations include gradient information—which coordinates are consumed forward, which are broadcast forward—lowering generates both the forward code and the backward code from the same IR.
...
(let-decl (output probs (i class))
  (/ (call exp (- (index logits (i class))
                  (reduction max (class) (index logits (i class)))))
     (reduction sum (class)
       (call exp (- (index logits (i class))
                    (reduction max (class) (index logits (i class))))))))
;; reduction-axes: (class), surviving: (i class), keepdims: true
```

Lowering reads the reduction axe `(class)` and maps it to `axis=1`. The `keepdims: true` annotation drives `keepdims=True` in the generated NumPy. The backward pass is generated by applying the Inversion Rule mechanically: every forward reduction (`max[class]`, `sum[class]`) becomes a backward broadcast. The generated gradient code distributes the softmax Jacobian without the programmer writing a single backward line.

A custom `@fn` rule overrides this automatic generation—but the coordinate names in the `@fn` signature must match the primal. The lowering pass verifies the match. If the primal declares `softmax[j]` and the custom rule declares `@fn softmax[k]`, the coordinate parameter names don't match. Error. The names prevent the mismatch.
