...
**2. Index arithmetic bounds.** `input[b, c, oh + kh, ow + kw]`—the compiler checks that `oh`, `kh`, `ow`, `kw` are declared coordinates. It cannot check that `oh + kh` stays within the input's spatial extent. That is a runtime fact, dependent on data: the compiler knows the coordinate domains but not the arithmetic relationship between them. At `oh = 63` and `kh = 4` on an input of spatial width 64, `oh + kh = 67` exceeds the bound. The compiler cannot predict all such combinations at compile time without running the loops.

The guarantee is not that the bounds are checked statically. The guarantee is that when the runtime does report an out-of-bounds error, the message names the coordinate and shows the arithmetic: `IndexError: oh + kh = 67 exceeds input width 64`. The error tells you which coordinate expression overflowed, which operands contributed, and what the bound was. The positional equivalent—`IndexError: dimension 3 out of bounds`—names a position. You then count dimensions, reconstruct the index arithmetic, and solve for which coordinate caused the overflow. The name doesn't prevent the error. It turns the diagnosis from a search into a sentence.
